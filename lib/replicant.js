...
Template = function(target, namespace) {
  ...
  subscribe = function(context, channelName) {
    ...
    return this.changesSubscription = postal.channel(channelName).subscribe(function(m) {
      var addName, childKey, control, lastIndex, newElement, newTree, parentKey;
      if (m.event !== "read") {
        ...
        if (m.event === "wrote") {
          if (control) {
            if (m.info.value instanceof ObjectWrapper || m.info.value instanceof ArrayWrapper) {
              newTree = context.template[m.key](self.html, m.info.value, parentKey, childKey);
              return $(context[m.key]).replaceWith(newTree);
            } else {
