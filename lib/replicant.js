...
DomProxy = (function() {
  ...
  function DomProxy(target, namespace) {
    ...
    template = namespace;
    this.element = target;
    self.crawl(template, this.element, function(namespace, key, child) {
      var member, prefix;
      console.log("" + template + " - " + namespace + "." + key);
      prefix = template + ".";
      member = namespace === template ? namespace : namespace.replace(prefix, "", "gi");
  ...
  DomProxy.prototype.crawl = function(namespace, element, callback) {
    ...
    if (element.children !== void 0 && element.children.length > 0) {
      return _(element.children).chain().each(function(child) {
        return self.crawl(fqn, child, callback);
      });
    }
  };
  return DomProxy;
