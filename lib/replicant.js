...
ObjectWrapper = function(target, onGet, onSet, namespace, addChild, removeChild) {
  var proxy;
...
ArrayWrapper = function(target, onGet, onSet, namespace, addChild, removeChild) {
  var proxy;
  proxy = new Proxy(this, target, onGet, onSet, namespace, addChild, removeChild);
  this.change_path = function(p) {
    return proxy.change_path(p);
...
Proxy = function(wrapper, target, onGet, onSet, namespace, addChild, removeChild) {
  var addChildPath, addToParent, createMemberProxy, createProxyFor, getCallback, path, proxy, removeChildPath, removeFromParent, setCallback, subject;
  proxy = {};
  ...
  this.push = function(value) {
    ...
    return setCallback(fqn, wrapper[subject.length - 1], void 0);
  };
  this.unshift = function(value) {
    var fqn, key;
    key = subject.length;
    ...
    createMemberProxy(key);
    fqn = buildFqn(path, key);
    return setCallback(fqn, wrapper[0], wrapper[1]);
  };
  this.pop = function() {
    var fqn, key, value;
  ...
  getCallback = function(key, value) {
    return onGet(wrapper, key, value);
  };
  setCallback = function(key, newValue, oldValue) {
    return onSet(wrapper, key, newValue, oldValue);
  };
  createProxyFor = function(writing, fqn, key) {
    ...
    if (writing || proxy[key] === void 0) {
      proxy[key] = onProxyOf(value, function() {
        return new ArrayWrapper(value, onGet, onSet, fqn, addChildPath, removeChildPath);
      }, function() {
        return new ObjectWrapper(value, onGet, onSet, fqn, addChildPath, removeChildPath);
      }, function() {
  ...
  createMemberProxy = function(key) {
    ...
    return Object.defineProperty(wrapper, key, {
      ...
      set: function(value) {
        ...
        return setCallback(fqn1, newValue, old);
      },
      configurable: true,
      enumerable: true
    });
...
replicant = {
  ...
  create: function(target, get, set, namespace) {
    ...
    proxy = onProxyOf(target, function() {
      return new ArrayWrapper(target, onGet, onSet, namespace);
    }, function() {
      return new ObjectWrapper(target, onGet, onSet, namespace);
    }, function() {
      return target;
    });
    (function() {
