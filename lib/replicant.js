...
Cartographer = function(target, namespace) {
  ...
  crawl = function(root, context, namespace, element) {
    ...
    if (element.children !== void 0 && element.children.length > 0) {
      createChildren = (function() {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          child = _ref[_i];
          _results.push(crawl(root, context, fqn, child));
        }
        return _results;
      })();
      return function(html, model, idx) {
        ...
        actual = id === "" ? idx : id;
        val = actual === fqn || actual === void 0 ? model : model[actual];
        if (val instanceof ArrayProxy) {
          ...
          return makeTag(html, tag, actual, element, list, root, model);
        } else {
          controls = (function() {
            var _i, _len, _results;
            _results = [];
          ...
          return makeTag(html, tag, actual, element, controls, root, model);
        }
      };
    } else {
      return function(html, model, idx) {
        ...
        actual = id === "" ? idx : id;
        if (!actual) {
          return makeTag(html, tag, actual, element, val, root, model);
        } else {
          val = actual === fqn || actual === void 0 ? model : model[actual];
          x = 0;
          if (val === void 0) {
            if (actual === "" || actual === void 0) {
              return makeTag(html, tag, "", element, model, root, model);
            } else {
              return makeTag(html, tag, "", element, model, root, model);
            }
  ...
  makeTag = function(html, tag, id, template, val, root, model) {
    ...
    element = html[tag](properties, val || properties.textContent);
    if (model[id]) {
      if (model[id].onclick) {
        console.log(model);
  ...
  this.map = function(model) {
    var fn;
    fn = crawl(model, this, namespace, this.element, this.map);
