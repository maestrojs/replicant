...
Cartographer = function(target, namespace) {
  var channel, conditionalCopy, copyProperties, crawl, createFqn, eventHandlers, makeTag, modelTargets, setupEvents, templateProperties, wireup;
  ...
  crawl = function(context, root, namespace, element) {
    ...
    if (element.children !== void 0 && element.children.length > 0) {
      ...
      return function(html, model, parentFqn, idx) {
        var actualId, call, controls, indx, list, myFqn, val, _ref;
        actualId = id === "" ? idx : id;
        ...
        if (val instanceof ArrayProxy) {
          ...
          for (indx = 0, _ref = val.length - 1; 0 <= _ref ? indx <= _ref : indx >= _ref; 0 <= _ref ? indx++ : indx--) {
            list.push((function() {
              ...
              for (_i = 0, _len = createChildren.length; _i < _len; _i++) {
                ...
                _results.push(call(html, val, myFqn, indx));
              }
              return _results;
            })());
          }
          return makeTag(html, tag, element, actualId, list, root, model);
        } else {
          controls = (function() {
            var _i, _len, _results;
            _results = [];
          ...
          return makeTag(html, tag, element, actualId, controls, root, model);
        }
      };
    } else {
      return function(html, model, parentFqn, idx) {
        var actualId, myFqn, val;
        actualId = id === "" ? idx : id;
        myFqn = createFqn(parentFqn, actualId);
  ...
  makeTag = function(html, tag, template, id, val, root, model) {
...
replicant = {
  ...
  create: function(target, get, set, namespace) {
    ...
    onGet = get || (get = function(key, value) {
      return channel.publish({
        key: key,
        value: value
      });
    });
    onSet = set || (set = function(key, value, old) {
      return channel.publish({
        key: key,
        value: value,
        original: old
